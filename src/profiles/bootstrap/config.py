...
DEFAULT_DB_URI = "sqlite+aiosqlite:///profiles.db"
...
def get_taskiq_broker_config() -> TaskiqBrokerConfig:
    return TaskiqBrokerConfig(
        environ.get(
            ...
            "profiles.bootstrap.entrypoints.tasks:bootstrap_broker",
        )
    )


def get_alembic_config() -> AlembicConfig:
    resource = files("profiles.infrastructure.persistence.alembic")
...
def get_uvicorn_config() -> UvicornConfig:
    return UvicornConfig(
        environ.get(
            "SERVER_FACTORY_PATH",
            "profiles.bootstrap.entrypoints.api:bootstrap_application",
        ),
